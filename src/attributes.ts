...
export interface PlayerDef {
  ...
  defRole?: string;
  // 守り方の指定。undefined = 自動（従来どおりスロット一致のマンマーク＋シェル）。
  //   "man"  … マンマーク。`markSlot` があればその相手を追いかける。
  //   "zone" … ゾーン。担当を追わず、守備ベース（シェル）を常に保つ。
  defMode?: "man" | "zone";
  // マンマークで誰を見るか（相手のスロット 0..4）。undefined = 同じスロットの相手。
  markSlot?: number;
...
export interface Tactics {
  ...
  /**
   * 各選手が基準とする立ち位置。**その瞬間に取る位置ではなく、開始位置と戻る先の基準**。
   * オープンスポットへの動き直し・スペーシング・マーク外しは従来どおり AI が決める。
   * 座標は「攻める(守る)リムから見た向き」で正規化して持つので、攻守でコートが
   * 入れ替わってもそのまま使える。
   *   x = コート横方向(m, 中央が0) / d = リムからミッドコート方向への距離(m)
   *   offBase … 攻撃時。formationSpots の7スポットに対応（0..4=外、5/6=中）。
   *   defBase … 守備時。守備シェルの5か所（slot 0=PG .. 4=C）。
   * 未設定ならコード側の既定値を使う。
   */
