...
export class Player {
  ...
  spotRim = 0;
  /** 計測用: オフボール更新でこのフレームに通った分岐名（probe から読む。試合には影響しない）。 */
  offBranch = "";
  /** この試合でゴール下(RIM_PTS_R 以内)から決めた得点。守備がダブルチームに行くかの判断に使う。 */
  rimPts = 0;
  ...
  readonly stats: Stats = { pts: 0, reb: 0, oreb: 0, dreb: 0, ast: 0, stl: 0, blk: 0, tov: 0, fgm: 0, fga: 0, tpm: 0, tpa: 0, ftm: 0, fta: 0, min: 0 };
  ...
  coolT = 0;
  // フォロースルーの全長(秒)。リリース直後に coolT の値を写して覚え、そこからの比で
  // 「伸ばし切り → 緩めて休めへ」の進み具合を出す（shootArms）。0 = まだ撃っていない。
  followDur = 0;
  // 放った向き（root ローカルの単位ベクトル）。ボールが手を離れた瞬間に1回だけ覚え、
  // フォロースルーの間ずっとこれを使う。⚠️ 飛んでいくボールやリムを毎フレーム狙うと、
  // 狙いが動いて腕がそれを追いかける（切り替えの瞬間に角速度が跳ねる）。
  followAimX = 0; followAimY = 1; followAimZ = 0;
  // 相手の手を払いのける動きの位相。ドリブル中のオフアームが進める。
  swipeT = 0;
  ...
  keepShieldT = 0;
  /**
   * 守って持つ（プロテクト）残り秒。詰められていて仕掛けていない時に入る。
   * 上半身を相手と反対へ回し、ボールを遠い手で構える＝**奪われにくいが前へ出られない**。
   * ⚠️ keepShieldT（下手なハンドラーの這うキープ）とは別物。こちらは誰でも入れる。
   */
  protectT = 0;
  ...
  openRollT = 0;
  /**
   * お膳立て: 良いパスからリズムよく受けた窓（秒）。
   * ⚠️ 以前はこの窓の間、**シュートの成功率に直接 +最大0.28** を足していた。
   *    ミドルの精度依存の幅が 0.42 しかないので、「誰から貰ったか」が
   *    「シュートが上手いか」より効いてしまう構造だった。成功率への上乗せは廃止し、
   *    **次の動作へ移りやすくなる**（捕球の硬直が短い / リリースが速い）形に変えた。
   */
  setupT = 0;
  /** お膳立ての質 0..1。捕球の硬直とシュートの溜めを縮める。成功率は直接いじらない。 */
  setupQuick = 0;
  baitT = 0;
  ...
  // スローインなどで**わざと**ライン外に立っている残り時間(s)。この間は clampCourt を
  // 素通しし、終わったら自分の足でコートへ戻る。⚠️ 位置を直接戻してはいけない（ワープ）。
  oobGraceT = 0;
  // 自陣へ全力で戻っている残り時間(s)。この間は守備の構え（中腰・ボールへ正対）をやめて
  // 走る向きへ体を向ける。⚠️ 胸がボールを向いたままだと後ろ歩きのクリップが選ばれ、
  //    中腰でジリジリ戻る絵になる（速度ではなく**向き**の問題）。
  transitT = 0;
  // ほぼ止まっている時間(s)。膠着（ハンドラーが止まる）に周りが反応するのに使う。
  stillT = 0;
  // マーク外しの軌道の曲がり(rad/s)。クイックな選手ほど大きく、円を描く/切り返す。
  shakeCurve = 0;
  // 守備が把握している「自分のマークの位置」。実位置へ遅れて追従する。
  // ⚠️ 実位置へ直結で寄せると、どんな外しにも付いて来て振り切れない。
  //    見ていない相手ほど遅れる（注目システム）。
  trackX = 0; trackZ = 0; trackOn = false;
  // 持ち場で回り続ける位相（フリーランニング）。止まった状態から仕掛けないため。
  freePhase = Math.random() * Math.PI * 2;
  // ── 注目システム（ai/attention.ts が毎フレーム書く）──
  gazeX = 0;        // いま見ている世界点
  gazeZ = 0;
  gazeKind = "ball" as "rim" | "ball" | "man" | "handler";
  gazeSwitchT = 0;  // 見る対象を変えた直後の「まだ見えていない」残り(0..1)
  // ── パスコースを潰されている（オフボールの攻撃側）──
  frontedT = 0;     // 自分とボールの間にマーカーが入っている残り時間(s)
  denyT = 0;        // 自分がレーンを塞いでいる残り時間(s)（守備側）
  backdoorT = 0;    // バックドアで裏を取っている残り時間(s)
