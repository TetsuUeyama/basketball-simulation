...
const REB_OUT = 2.4;
/**
 * 攻撃のベース位置の既定値。攻めるリムから見た {x, d}（d = ミッドコート方向への距離 m）。
 * ⚠️ ペリメーターの4人は 3Pライン(6.75m) のすぐ上。中は1人だけで、もう1人は
 *    ショートコーナーへ出してヘルプを引き出す設計。
 * ⚠️ コーナーは x=±6.85。コートのクランプは ±7.0 なので、これ以上外は届かない。
 */
...
export const OFF_BASE_DEFAULT: { x: number; d: number }[] = [
  ...
  //    （実測: ペリメーターのオフボールでラインより外は 52.5%、
  //    　6.0〜6.75m の「あと一歩」が 9.0%）。
  //    0.5m の余裕を持たせ、**受けてそのまま打てば3Pになる**位置で待たせる。
  // ⚠️ **スポット同士の間隔を必ず確認すること**。以前はウイング(±5.80, 4.35)と
  //    コーナー(±6.85, 2.37)が同じ 7.25m の円弧上に 18度差で並んでおり、
  //    **同じ側で 2.24m しか離れていなかった**。2人が正しく自分の持ち場に立つだけで
  //    団子になる配置で、実測でも味方同士が2m未満のフレームが 51.5% あった。
  //    `bestOpenSpot` の重み（ベースへの寄り・3P能力によるコーナー選好）を3回
  //    調整しても改善しなかったのは、**選び方ではなく置き場所の問題**だったから。
  //    ウイングを45度へ、コーナーをベースライン寄りへ下げて、最小間隔を 3.51m にした。
  { x: 0, d: 7.25 },       // 0 トップ（リムから 7.25m）
  { x: -5.13, d: 5.13 },   // 1 左ウイング（45度・7.25m）
  { x: 5.13, d: 5.13 },    // 2 右ウイング（45度・7.25m）
  // ⚠️ コーナーは d < 1.414 の**直線区間**に置く。ここは |x| >= THREE_CORNER_X(6.6) が
  //    3Pの条件なので、6.9 なら半径(7.00m)でも直線でも確実に3Pになる。
  { x: -6.90, d: 1.20 },   // 3 左コーナー
  { x: 6.90, d: 1.20 },    // 4 右コーナー
  { x: -2.6, d: 1.3 },     // 5 左ローブロック（ダンカースポット）
  // ⚠️ 以前は x=6.2 (リムから 6.26m) で、3Pライン(6.75m)のすぐ手前だった。
  //    スペーシングの押し出しで簡単に外へ出てしまい、「3Pが下手なビッグが
  //    アークに立っている」絵になっていた。ゴール下寄りへ引き込む。
  { x: 3.4, d: 1.0 },      // 6 右ショートコーナー（中は空けるが、ゴール下の高さに留まる）
];
export class Game {
  ...
  formationSpots(team: number): Vector3[] {
    ...
    // ⚠️ 既定値。チームごとに `TACTICS[team].offBase` があればそちらを使う
  ...
  prefersPost(p: Player): boolean {
    ...
    // ⚠️ 残るビッグも、**3Pが下手なら外へ出さない**。ここで false を返すと、その選手は
    //    自分のスロットのスポット(3/4 = コーナー3、リムから 6.95m)に立つ。つまり
    //    「3Pが上手くないビッグが3Pラインに突っ立っている」状態になる
    //    （実測: PF の 43.6% / C の 49.6% がアーク付近に居た）。
    //    外へ広げてよいのは**本物のストレッチビッグだけ**。それ以外はポスト側
    //    （アンカー=ローブロック / もう1人=ショートコーナー）に置く。
    return rate(p.attr.threeAcc) < STRETCH_THREE;
  }
  ...
  homeSpotIdx(p: Player): number {
    ...
    // ⚠️ スポット配列は **[トップ, 左ウイング, 右ウイング, 左コーナー, 右コーナー,
    //    左ローブロック, 右ショートコーナー]** というペリメーター順の並びで、
    //    ポジション順(PG..C)ではない。ここに p.slot をそのまま添字として使うと、
    //    **PF(slot 3) は左コーナー / C(slot 4) は右コーナー**へ送られる。
    //    ポスト役から外れたビッグ（L精度80以上のストレッチ）が必ずコーナーに立つ、
    //    という状態になっていた（実測: C の 2.3% / PF の 5.7% が該当し、
    //    先発は能力の高い選手が選ばれるのでちょうどこの層が当たる）。
    //    ストレッチビッグの居場所は**コーナーではなくウイング**。コーナーは
    //    ガード/ウイングの持ち場として空けておく。
    if (this.isBig(p)) return p.pos.x >= 0 ? 2 : 1;   // 自分の側のウイングへ
    return p.slot;
  }
