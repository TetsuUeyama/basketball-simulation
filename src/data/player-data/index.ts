...
// （id = 配列インデックス）。
//   - identity.ts : [id, name, role, heightCm, hand, look[skin,hair,style,hairNo], weightKg, posMask]
...
//  weightKg, posMask]
// posMask = 守れるポジションのビット和（C=1 / PF=2 / SF=4 / SG=8 / PG=16）。
...
export const PLAYER_DB: DbPlayer[] = IDENTITY.map((idn, i): DbPlayer => {
  ...
  return [idn[1], idn[2], idn[3], rt[1], ab[1], rt[2], idn[4], idn[5], idn[6], idn[7]];
});
